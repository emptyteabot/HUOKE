...
const capabilities = [
  {
    icon: Cpu,
    title: 'AI 动态意图打分引擎',
    detail: '抛弃精准搜索自然流，不再靠关键词生成找“可能相关”的人。系统把公开讨论和表单输入统一送入动态评分，实时判断痛点迫切度、预算范围和可推进动作。',
  },
  {
    icon: Filter,
    title: '长上下文预算解析',
  ...
  },
  {
    icon: Globe,
    title: '高净值线索防火墙',
    detail: '低预算、泛咨询和白嫖样本先被拦截，只有预算达标、痛点明确、能写进日历的线索进入销售视野。',
  },
  {
    icon: LayoutDashboard,
    title: '结构化交付工作台',
    detail: '每条通过防火墙的线索都保留来源、上下文、预算判断、阶段和下一步动作，销售不用再从一堆链接里猜。',
  },
];
...
export default function ProductPage() {
  return (
    <MarketingPageShell
      ...
      typeLine="抛弃关键词生成，改用 AI 动态意图打分和预算防火墙。"
              ...
              <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 p-5 shadow-2xl">
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex items-center justify-between rounded-lg border border-slate-700/50 bg-slate-800/50 p-3">
                    ...
                    <span className="font-mono text-xs text-green-400">判定痛点迫切度</span>
                  </div>
                  <div className="flex items-center justify-between rounded-lg border border-slate-700/50 bg-slate-800/50 p-3">
                    <span className="flex items-center gap-2 text-xs text-slate-300">
                    ...
                    <span className="font-mono text-xs text-red-400">低净值拦截</span>
                  </div>
                </div>
              </div>
            </div>
